    ...
    function GenerateReportUrl()
    {
		...
		mifgaNum=GenerateMifgaNum();
		var now=new Date();
		if (CheckDateNotInFuture(crimeTimeMilisec)==false)
		{
			crimeTimeMilisec=now.getTime();
		}
...
/*    function TogglePicSize(id) 
	...
	function CheckDateNotInFuture(milisec)
	{
		var now=new Date();
		if (now.getTime()-milisec <=0 )
		{
			return false;
		}
		else
	...
	function CheckDate()
	{
		...
		{
			var timeMilisec=$('#picker').mobiscroll('getDate').getTime();
			var res=CheckDateNotInFuture(timeMilisec);
			if (res==false)
			{
